...
import { faArrowRight, faChevronDown, faSignOutAlt, faChartBar } from '@fortawesome/free-solid-svg-icons';
...
import plan3 from '../assets/img/10.png';
...
function Dashboard() {
  ...
  const [plans] = useState([
    { nom: 'Culture Générale Africaine', miniature: plan1 },
    { nom: 'Quiz de la Nature Générale Africaine', miniature: plan2 },
    { nom: 'Safari Quiz : Explorez la Faune et la Nature Africaines', miniature: plan3 },
  ]);

  // États pour l'animation des scores
  ...
  useEffect(() => {
    ...
    const stepTime = 20; // Intervalle entre chaque incrément
    ...
    const timer = setInterval(() => {
      ...
      if (currentScore >= scoreTotal && currentStat1 >= 5325 && currentStat2 >= 4006 && currentStat3 >= 4000) {
        clearInterval(timer);
      }
    }, stepTime);
          ...
          <div className="absolute bottom-0 w-full p-5">
            <div className="group border-2 border-dashed border-afrik-red p-2 rounded-2xl">
              <div className="bg-afrik-red text-xl rounded-full px-3 py-1 flex justify-center items-center shadow-md shadow-afrik-red/50 group-hover:bg-white group-hover:text-afrik-red group-hover:cursor-pointer">
                <FontAwesomeIcon icon={faSignOutAlt} className="h-6 w-4 fill-current text-white group-hover:text-afrik-red" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-[90%] ml-[10%] flex justify-start items-start space-x-3 p-4">
        ...
        >
          ...
          <div className="flex justify-between items-center">
            <span className="text-xl font-bold">Bienvenue, {joueur.prenom} {joueur.nom}!</span>
            <span className="text-afrik-red text-opacity-45 text-lg">#1</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex items-center justify-between space-x-4">
                <div>
                  ...
                  <p className="text-sm text-white/70">
                    Représenté par la figure du vieillard sage, ce niveau incarne la maîtrise, l'expérience et la sagesse. Les experts sont des mentors et des leaders, capables d'analyser les situations complexes et de transmettre leur savoir avec clarté et perspicacité.
                  </p>
                </div>
            ...
            >
              <div className="bg-black bg-opacity-35 rounded-3xl p-4 text-center">
                Vous avez débloqué le niveau <span className="text-afrik-light-red font-bold">Expert</span>
              </div>
              <div className="bg-black bg-opacity-35 rounded-3xl p-4 text-center">
                Vous avez joué à <span className="text-afrik-red font-bold">{totaljouer}</span> partie au total !
              </div>
            </motion.div>
          </div>
          {/* Sections plans et dernière partie */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="col-span-2 grid grid-cols-1 grid-rows-2 gap-6">
              ...
              >
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-afrik-red text-opacity-45 text-sm">voir plus</span>
                </div>
                <div className="grid grid-cols-3 gap-4">
                  {plans.map((plan, index) => (
                    ...
                    >
                      <img src={plan.miniature} alt={plan.nom} className="w-full h-32 object-cover rounded-t-3xl" />
              ...
              >
                <div className="flex items-center space-x-4">
                  <img src={dernierePartie.miniature} alt={dernierePartie.nom} className="h-20 rounded-lg" />
                  <div>
                    <div className="font-bold">{dernierePartie.nom}</div>
                    <div className="text-sm text-white/70">{dernierePartie.niveau}</div>
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-afrik-red text-opacity-45 font-semibold">Score</div>
                  <div className="text-2xl font-bold">{dernierePartie.score}</div>
                </div>
              </motion.div>
            </div>
            {/* Mes statistiques */}
            ...
            >
              <div className="flex justify-between items-center mb-4">
                ...
                <FontAwesomeIcon icon={faArrowRight} className="h-6 text-afrik-red" />
              </div>
              <div className="relative w-full h-64">
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                  <div className="border-dashed border-afrik-red/45 border-2 p-6 rounded-full text-center">
                    <div className="text-afrik-red/45 font-semibold text-lg">Score total</div>
        ...
        {/* Barre latérale droite améliorée */}
        <motion.div
          initial={{ x: 50, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
        ...
        >
          <div className="bg-black bg-opacity-35 py-4 shadow-inner rounded-full w-full text-white flex flex-col items-center space-y-4">
            <a href="#">
              <div className="h-10 flex justify-center items-center">
                <FontAwesomeIcon icon={faChartBar} className="h-8 w-6 text-white hover:text-afrik-red" />
              </div>
            </a>
